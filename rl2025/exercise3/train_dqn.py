...
def train(env: gym.Env, config, output: bool = True) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict]:
    ...
    with tqdm(total=config["max_timesteps"]) as pbar:
        while timesteps_elapsed < config["max_timesteps"]:
            ...
            if timesteps_elapsed % config["eval_freq"] < episode_timesteps:
                ...
                # print("Plotting DQN...")
                # x = (config["batch_size"] * np.arange(1, len(run_data["q_loss"]) + 1)).tolist()
                # x1 = (config["batch_size"] * np.arange(1, len(run_data["train_ep_returns"]) + 1)).tolist()
                # fig, axis = plt.subplots(1,2)
                # axis[0].plot(x, run_data["q_loss"], '-', alpha=0.7)
                # axis[0].set_title('QLoss')

                # axis[1].plot(x1, run_data["train_ep_returns"])
                # axis[1].set_title('Returns')
                # plt.savefig(f'./{}{timesteps_elapsed}.png')
    ...
    if config["plot_loss"]:
        q_plot_save_name = f'{config["save_filename"]}/Q_loss.png'
        return_plot_save_name = f'{config["save_filename"]}/eval_returns.png'
        train_return_save_name = f'{config["save_filename"]}/train_returns.png'
        ...
        plt.savefig(q_plot_save_name)


        plt.plot(run_data["train_episodes"], run_data["train_ep_returns"], "-", alpha=0.7)
        plt.xlabel("Episodes", fontsize=30)
        plt.ylabel("Train Returns", fontsize=30)
        plt.xticks(fontsize=25)
        plt.yticks(fontsize=25)
        plt.tight_layout(pad=0.3)

        plt.savefig(train_return_save_name)


        # x = np.arange(1, len(eval_returns_all) + 1).tolist()
        # plt.plot(x, eval_returns_all, "-", alpha=0.7)
        # plt.xlabel("Timesteps", fontsize=30)
        # plt.ylabel("Eval Returns", fontsize=30)
        # plt.xticks(fontsize=25)
        # plt.yticks(fontsize=25)
        # plt.tight_layout(pad=0.3)

        # plt.savefig(return_plot_save_name)
