        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        while True:
            ...
            for s in range(len(self.mdp.states)):
                v = V[s]
                V[s] = max(sum(self.mdp.P[s, a, s_next]*
                               (self.mdp.R[s, a, s_next] + self.gamma * V[s_next])
                               for s_next in range(len(self.mdp.states))) for a in range(len(self.mdp.actions)))
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        for s in range(num_states):
            action_val = np.zeros(num_actions)
            for a in range(num_actions):
                action_val[a] = sum(
                    self.mdp.P[s,a,s_next] * (self.mdp.R[s,a,s_next] + self.gamma * V[s_next]) for s_next in range(num_states)
                )
